...
- Auto. Select :guilabel:`Tools -> Auto Upload Mode -> Enable` Note that only boards with auto upload circuit built-in (BW16-TypeC) can use the Auto Upload Mode, otherwise it will back to normal upload mode and wait for 5 seconds.
