...
Initializes the PowerMode settings for device, including power mode, multiple wake up sources, retention and related source settings.
...
wakeup_setting: pointer to array of settings, one per wakeup source bit set. Order should match the bit positions: [AON_TIMER, AON_GPIO, RTC, COMP]
